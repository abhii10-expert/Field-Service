...
export default class CpeAiAssistant extends LightningElement {
  ...
  "CpeItems": [
    ...
    async handleSend() {
        ...
        if (lower.includes('nearby') || lower.includes('peer') || lower.includes('technician') || lower.includes('near') || lower.includes('anybody')) {
            this.isTyping = true;
            try {
                ...
                if (result.peers && result.peers.length > 0) {
                    ...
                    // --- DEMO MOCK FALLBACK (Product-Specific filtering) ---
                    let mockPeers = [];
                    if (prod === 'Modem') {
                        mockPeers = [{ name: 'Tech Sarah', distance: '1.2 km away', quantity: 10 }];
                    } else if (prod === 'Receiver') {
                        mockPeers = [{ name: 'John Miller', distance: '4.5 km away', quantity: 3 }];
                    } else {
                        // General search
                        mockPeers = [
                            { name: 'Tech Sarah', distance: '1.2 km away', quantity: 5 },
                            { name: 'John Miller', distance: '4.5 km away', quantity: 3 }
                        ];
                    }
