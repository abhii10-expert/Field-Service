...
export default class CpeAiAssistant extends LightningElement {
  ...
  "CpeItems": [
    ...
    async handleSend() {
        ...
        if (nearbyKeywords.some(keyword => lower.includes(keyword))) {
            this.isTyping = true;
            try {
                let prod = lower.includes('modem') ? 'Modem' : (lower.includes('receiver') ? 'Receiver' : null);
    ...
    processAiResponse(parsed) {
        ...
        if (parsed.stateChanges) {
            parsed.stateChanges.forEach(change => {
                const val = change.newUnits;
                if (change.label === 'TV + RECEIVER') {
                    const newQty = parseInt(val);
                    if (this.tvCurrent !== newQty) diffDetected = true;
                    this.tvCurrent = newQty;
                }
                if (change.label === 'WI-FI ACCESS POINT') {
                    const newQty = parseInt(val);
                    if (this.wifiCurrent !== newQty) diffDetected = true;
                    this.wifiCurrent = newQty;
                }
                if (change.label === 'MODEM') {
                    if (this.modemCurrent !== val) diffDetected = true;
                    this.modemCurrent = val;
                }
            });
        }

        if ((diffDetected || (parsed.stateChanges && parsed.stateChanges.length > 0)) && parsed.message) {
            const combinedItems = (parsed.stateChanges || []).map(change => {
                const isModem = (change.label === 'MODEM');
                const label = change.label;
                
                let baseOldVal = null;
                if (label === 'TV + RECEIVER') baseOldVal = this.baseTv;
                else if (label === 'WI-FI ACCESS POINT') baseOldVal = this.baseWifi;
                else if (label === 'MODEM') baseOldVal = this.baseModem;

                const newVal = change.newUnits;
                const isRemoval = !isModem && (parseInt(newVal) < baseOldVal);
                const removedQuantity = isRemoval ? (baseOldVal - parseInt(newVal)) : 0;
                ...
                return {
                    ...
                    label: label,
                    oldModem: isModem ? baseOldVal : null,
                    newModem: isModem ? newVal : null,
                    oldUnits: !isModem ? baseOldVal : null,
                    newUnits: !isModem ? newVal : null
                };
            });
