...
export default class CpeAiAssistant extends LightningElement {
  ...
  "CpeItems": [
    ...
    focusChat() {
        const chatSection = this.template.querySelector('.ai-assistant');
        if (chatSection) {
    ...
    focusTop() {
        const topSection = this.template.querySelector('.cpe-container');
        if (topSection) {
            topSection.scrollTo({ top: 0, behavior: 'smooth' });
        }
    }
