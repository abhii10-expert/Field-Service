...
import chatbotAvatarResource from '@salesforce/resourceUrl/chatbotAvatar';

export default class CpeAiAssistant extends LightningElement {
    get personaAvatarUrl() {
        return chatbotAvatarResource;
    }
  ...
  "CpeItems": [
    ...
    @track messages = [
        {
            ...
            text: "Hi, I'm Astro, your Field Service Assistant! I've scanned the current work order. How can I help you adjust this setup?",
            textLines: ["Hi, I'm Astro, your Field Service Assistant! I've scanned the current work order. How can I help you adjust this setup?"],
            isSetupIntro: true,
            setupItems: [
                { label: '2 TV Receiver' },
                { label: 'Wired Modem' },
    ...
    async handleSend() {
        ...
        if (this.currentFlow === 'AddReceivers') {
            ...
            this.scannerQuantityToSelect = quantity;
            this.simulateScannerForRemoval();
            return;
        }
    ...
    confirmCartOrder() {
        ...
        setTimeout(() => {
            ...
            ctaText: "How can I help you adjust this setup?",
            wrapperClass: 'chat-wrapper ai',
            bubbleClass: 'chat-bubble ai'
        });
        }, 1500);
    ...
    @track scannerLineClass = 'scanner-line';
    @track scannerStatusText = 'Scanning nearby equipment...';
    @track showScannedDevices = false;
    @track scannedDevices = [];
    @track scannerQuantityToSelect = 1;

    simulateScannerForRemoval() {
        this.isScannerOpen = true;
        this.scannerLineClass = 'scanner-line';
        this.scannerStatusText = `Scanning for ${this.removeProduct}s...`;
        this.showScannedDevices = false;
        this.scannedDevices = [];

        // Determine how many we have currently
        let total = this.removeProduct === 'TV Receiver' ? this.tvCurrent : this.wifiCurrent;
        ...
        for (let i=0; i<total; i++) {
            devices.push({
                ...
                type: this.removeProduct,
                sn: 'SN-' + Math.random().toString(36).substr(2, 8).toUpperCase(),
                selected: false,
                cssClass: 'scanned-device'
            });
        }
        this.scannedDevices = devices;

        setTimeout(() => {
            this.scannerLineClass = 'scanner-line finished';
            this.scannerStatusText = `${total} ${this.removeProduct}(s) detected.`;
            this.showScannedDevices = true;
            
            // Auto-select the requested quantity
            setTimeout(() => {
                let toSelect = parseInt(this.scannerQuantityToSelect, 10);
                if (isNaN(toSelect)) toSelect = 1;
                
                let count = 0;
                this.scannedDevices = this.scannedDevices.map(d => {
                    if (count < toSelect) {
                        d.selected = true;
                        d.cssClass = 'scanned-device selected';
                        count++;
                    }
                    return d;
                });
    ...
    get selectedDeviceCount() {
        return this.scannedDevices.filter(d => d.selected).length;
    }

    get isConfirmScannerDisabled() {
        return this.selectedDeviceCount === 0;
    }
    ...
    handleDeviceSelect(event) {
        ...
        this.scannedDevices = this.scannedDevices.map(d => {
            if (d.id === id) {
                ...
                d.cssClass = d.selected ? 'scanned-device selected' : 'scanned-device';
            }
            return d;
        });
    }
    ...
    closeScanner() {
        ...
        this.addAiMessage('Device scan cancelled.');
    }

    confirmScannerRemoval() {
        this.isScannerOpen = false;
        const count = this.selectedDeviceCount;
        this.sendToAgent(`Remove ${count} ${this.removeProduct}`);
    }
}
